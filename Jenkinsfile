...
// which are part of the BitDollar repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="bitdollar-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /bitdollar-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "bitdollarpay-bitdollar/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from bitdollarpay-bitdollar/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'bitdollarpay-bitdollar/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from bitdollarpay-bitdollar/develop"
            }
          }

          if (hasCache) {
            sh "cd /bitdollar-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /bitdollar-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /bitdollar-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /bitdollar-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /bitdollar-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /bitdollar-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /bitdollar-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
