...
*Setup instructions for a Gitian build of BitDollar Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the BitDollar
...
- [Building BitDollar Core](#building-bitdollar-core)
...
Clone the git repositories for BitDollar Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/bitdollarpay/bitdollar
...
in the BitDollar Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building BitDollar Core
----------------

To build BitDollar Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the BitDollar Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/bitdollar/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/bitdollarpay/bitdollar
...
For example:
```bash
URL=https://github.com/crowning-/bitdollar.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit bitdollar=${COMMIT} --url bitdollar=${URL} ../bitdollar/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bitdollar=${COMMIT} --url bitdollar=${URL} ../bitdollar/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit bitdollar=${COMMIT} --url bitdollar=${URL} ../bitdollar/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the bitdollar git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../bitdollar/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/bitdollar
SIGPATH=/some/root/path/bitdollar-detached-sigs

./bin/gbuild --url bitdollar=${BTCPATH},signature=${SIGPATH} ../bitdollar/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bitdollar-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bitdollar-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/bitdollar-osx-build.assert
```
...
[bitdollar/gitian.sigs](https://github.com/bitdollarpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
