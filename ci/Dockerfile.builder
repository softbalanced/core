...
# bitdollar_hash
RUN git clone https://github.com/bitdollarpay/bitdollar_hash
RUN cd bitdollar_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} bitdollar
RUN useradd -u ${USER_ID} -g bitdollar -s /bin/bash -m -d /bitdollar bitdollar
...
RUN mkdir /bitdollar-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /bitdollar-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /bitdollar-src
