...
pkgdatadir = $(datadir)/bitdollarcore
pkgincludedir = $(includedir)/bitdollarcore
pkglibdir = $(libdir)/bitdollarcore
pkglibexecdir = $(libexecdir)/bitdollarcore
...
CONFIG_HEADER = $(top_builddir)/src/config/bitdollar-config.h
CONFIG_CLEAN_FILES = libbitdollarconsensus.pc share/setup.nsi \
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	$(srcdir)/libbitdollarconsensus.pc.in \
	...
	$(top_srcdir)/src/config/bitdollar-config.h.in \
...
ACLOCAL = ${SHELL} /tmp/BitDollar/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /tmp/BitDollar/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /tmp/BitDollar/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /tmp/BitDollar/build-aux/missing automake-1.15
AWK = gawk
BDB_CFLAGS = 
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = bitdollar-cli
BITCOIN_DAEMON_NAME = bitdollard
BITCOIN_GUI_NAME = bitdollar-qt
BITCOIN_TX_NAME = bitdollar-tx
...
COPYRIGHT_HOLDERS_FINAL = The BitDollar Core developers
COPYRIGHT_HOLDERS_SUBSTITUTION = BitDollar Core
COPYRIGHT_YEAR = 2021
CPP = gcc -E
...
CXXFLAGS = -g -O2 -Wall -Wextra -Wformat -Wvla -Wformat-security -Wshadow -Wno-unused-parameter -Wno-implicit-fallthrough
CYGPATH_W = echo
DEFS = -DHAVE_CONFIG_H
DEPDIR = .deps
DLLTOOL = false
...
MAINT = 
MAKEINFO = ${SHELL} /tmp/BitDollar/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL = 
OTOOL64 = 
PACKAGE = bitdollarcore
PACKAGE_BUGREPORT = https://github.com/bitdollarpay/bitdollar/issues
PACKAGE_NAME = BitDollar Core
PACKAGE_STRING = BitDollar Core 1.0.0
PACKAGE_TARNAME = bitdollarcore
PACKAGE_URL = https://bitdollar.org/
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /tmp/BitDollar
abs_srcdir = /tmp/BitDollar
abs_top_builddir = /tmp/BitDollar
abs_top_srcdir = /tmp/BitDollar
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /tmp/BitDollar/build-aux/install-sh
...
pkgconfig_DATA = libbitdollarconsensus.pc
...
DIST_CONTRIB = $(top_srcdir)/contrib/bitdollar-cli.bash-completion \
	       $(top_srcdir)/contrib/bitdollar-tx.bash-completion \
	       $(top_srcdir)/contrib/bitdollard.bash-completion \
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info \
  leveldb_baseline.info test_bitdollar_filtered.info total_coverage.info \
  baseline_filtered.info rpc_test.info rpc_test_filtered.info \
  leveldb_baseline_filtered.info test_bitdollar_coverage.info test_bitdollar.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/BitDollar-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/bitdollar-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/bitdollar-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/bitdollar-config.h
$(top_srcdir)/src/config/bitdollar-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/bitdollar-config.h src/config/stamp-h1
libbitdollarconsensus.pc: $(top_builddir)/config.status $(srcdir)/libbitdollarconsensus.pc.in
...
$(OSX_APP)/Contents/MacOS/BitDollar-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/BitDollar-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/BitDollar-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_bitdollar.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_bitdollar -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_bitdollar_filtered.info: test_bitdollar.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#rpc_test.info: test_bitdollar_filtered.info
...
#test_bitdollar_coverage.info: baseline_filtered_combined.info test_bitdollar_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_bitdollar_filtered.info -o $@

#total_coverage.info: baseline_filtered_combined.info test_bitdollar_filtered.info rpc_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_bitdollar_filtered.info -a rpc_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_bitdollar.coverage/.dirstamp:  test_bitdollar_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_bitdollar.coverage/.dirstamp total.coverage/.dirstamp

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf coverage_percent.txt test_bitdollar.coverage/ total.coverage/ qa/tmp/ cache/ $(OSX_APP)
