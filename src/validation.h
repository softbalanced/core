...
//! Discourage users to set fees higher than this amount (in Cent) per kB
static const CAmount HIGH_TX_FEE_PER_KB = 0.00001 * COIN;
//! -maxtxfee will warn if called with a higher fee than this amount (in Cent)
 ...
 *  We have 4 times smaller block times in BitDollar, so we need to push 4 times more invs per 1MB. */
...
/** Absolute maximum transaction fee (in Cent) used by wallet and mempool (rejects high fee in sendrawtransaction) */
