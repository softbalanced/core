...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (fLiteMode) return; // ignore all BitDollar related functionality
...
/*
    ...
    allow endless transaction that would bloat BitDollar and make it unusable. To
...
{
    if (fLiteMode) return;        // disable all BitDollar specific functionality
