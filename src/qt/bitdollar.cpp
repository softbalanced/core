...
{
    return QCoreApplication::translate("bitdollar-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in bitdollar.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in bitdollar.qrc)
...
/** Class encapsulating BitDollar Core startup and shutdown.
...
/** Main BitDollar application object */
...
void BitcoinApplication::initializeResult(bool success)
{
    ...
    if(success)
    {
        ...
        // bitdollar: URIs or payment requests:
...
{
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. BitDollar Core can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(bitdollar);
    Q_INIT_RESOURCE(bitdollar_locale);
    ...
    /// 6. Determine availability of data directory and parse bitdollar.conf
    ...
    // bitdollar: links repeatedly have their payment requests routed to this process:
