...
const char * const BITCOIN_CONF_FILENAME = "bitdollar.conf";
const char * const BITCOIN_PID_FILENAME = "bitdollard.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "bitdollar" is a composite category enabling all BitDollar-related debug output
                if(ptrCategory->count(std::string("bitdollar"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BitDollar
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BitDollar
    // Mac: ~/Library/Application Support/BitDollar
    // Unix: ~/.bitdollar
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitDollar";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BitDollar";
#else
    // Unix
    return pathRet / ".bitdollar";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty bitdollar.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override bitdollar.conf
